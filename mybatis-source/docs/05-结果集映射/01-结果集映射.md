...
```mermaid
flowchart TB
    PS[PreparedStatement.execute] --> RS[ResultSet<br/>JDBC 结果集]
    RS --> RSH[ResultSetHandler<br/>handleResultSets]
    RSH --> RSW[ResultSetWrapper<br/>获取列名、类型信息]
    RSW --> RM[ResultMap<br/>映射配置]
    
    RM --> |遍历每一行| LOOP{循环处理}
    LOOP --> CREATE[创建结果对象<br/>ObjectFactory]
    CREATE --> MAP[属性映射<br/>自动映射/显式映射]
...
```mermaid
flowchart TB
    RSH[ResultSetHandler<br/>结果集处理入口] --> RSW[ResultSetWrapper<br/>包装 ResultSet]
    RSH --> RM[ResultMap<br/>映射配置]
    RSW --> TH[TypeHandler<br/>类型转换]
    RM --> OF[ObjectFactory<br/>对象创建]
```
