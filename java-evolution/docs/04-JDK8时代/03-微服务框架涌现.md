...
graph TB
    subgraph Spring Cloud Netflix
        A[Eureka] -->|服务发现| B[服务注册中心]
        C[Ribbon] -->|负载均衡| D[客户端负载]
        E[Feign] -->|声明式调用| F[HTTP 客户端]
        G[Hystrix] -->|熔断器| H[服务保护]
        I[Zuul] -->|网关| J[API 网关]
        K[Config] -->|配置中心| L[集中配置]
    end
```
...
flowchart TD
    ...
    B -->|高性能 RPC| C[Dubbo]
    B -->|通用性优先| D{团队背景?}
    D -->|熟悉 Spring| E[Spring Cloud]
    D -->|熟悉阿里技术| F[Dubbo + Spring Cloud Alibaba]
