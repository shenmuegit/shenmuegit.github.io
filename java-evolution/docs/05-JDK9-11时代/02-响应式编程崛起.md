...
timeline
    ...
    section 早期探索
        2013 : RxJava 1.0
        2014 : Reactive Streams 规范
    section 成熟发展
        2016 : RxJava 2.0
        2017 : 📍 Project Reactor
             : Spring 5 WebFlux
    section 标准化
        2017 JDK 9 : Flow API
```
...
sequenceDiagram
    ...
    P->>S: onNext(item1)
    P->>S: onNext(item2)
    Note over P,S: ...发送 10 个元素
    S->>P: request(5)
    P->>S: onNext(item11)
    Note over P,S: 订阅者控制消费速率
```
...
graph TB
    subgraph Spring MVC
        A1[Servlet 容器] --> B1[DispatcherServlet]
        B1 --> C1[同步 Controller]
        C1 --> D1[阻塞 I/O]
    end
...
graph TB
    subgraph JDK 特性
        A[JDK 8 Lambda] --> B[流畅的操作符 API]
        C[JDK 8 CompletableFuture] --> D[异步编程基础]
        E[JDK 9 Flow API] --> F[标准化接口]
    end
